...
import org.videolan.vlc.betav7neon.gui.AudioUtil;
...
public class AudioService extends Service {
    ...
    public void setUpRemoteControlClient() {
        Context context = GlobalVariables.gAppContext;
    ...
    public void onDestroy() {
        super.onDestroy();
        Log.i("zjj", "AudioService stop()");
        stop();
        
        Log.i("zjj", "AudioService mWakeLock.isHeld()");
        if (mWakeLock.isHeld())
            mWakeLock.release();
        
        Log.i("zjj", "AudioService unregisterReceiver(serviceReceiver)");
        unregisterReceiver(serviceReceiver);
        
        Log.i("zjj", "AudioService unregisterReceiver(mRemoteControlClientReceiver)");
    ...
    private final BroadcastReceiver serviceReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if( mLibVLC == null ) {
                Log.w("zjj", "AudioService Intent received, but VLC is not loaded, skipping.");
                return;
            }
...
//                context.startActivity(iVlc);
             ...
             * Sleep
             */
            if (action.equalsIgnoreCase(GlobalVariables.SLEEP_INTENT)) {
                stop();
            }
        }
    };
            ...
            switch (msg.getData().getInt("event")) {
                case EventHandler.MediaPlayerPlaying:
                    ...
                    MediaDatabase dbManager = MediaDatabase
                            .getInstance(GlobalVariables.gAppContext);
                    Media m = dbManager.getMedia(GlobalVariables.gAppContext,
                            location);
                    /**
                ...
                case EventHandler.MediaPlayerEncounteredError:
                    ...
                    break;
                default:
                    Log.e(TAG, "Event not handled:" + msg.getData().getInt("event"));
                    break;
            }
        }
    };
    ...
    private void handleVout() {
    	Log.i("zjj", "Obtained video track");
        mMediaList.clear();
        hideNotification();
        ...
        Log.i("zjj", "AudioService handleVout()");
        VideoPlayerActivity.start(GlobalVariables.gAppContext, mCurrentMedia.getLocation(), mCurrentMedia.getTitle(), true);
    }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case SHOW_TOAST:
                    ...
                    Toast.makeText(GlobalVariables.gAppContext, text, duration).show();
                    break;
            }
        }
    };
    ...
    private void stop() {
    	try{
    		if(mLibVLC.isPlaying()){
    			mLibVLC.stop();
                Log.i("zjj", "AudioService mLibVLC.stop()");
    		}
    	}catch(Exception e){
    		Log.i("zjj", "AudioService " + e.toString());
    	}
        
        mEventHandler.removeHandler(mVlcEventHandler);
        Log.i("zjj", "AudioService mEventHandler.removeHandler(mVlcEventHandler)");
        
        setRemoteControlClientPlaybackState(EventHandler.MediaPlayerStopped);
        Log.i("zjj", "AudioService setRemoteControlClientPlaybackState(EventHandler.MediaPlayerStopped)");
...
//        hideNotification();
        executeUpdate();
        Log.i("zjj", "AudioService executeUpdate()");
    ...
    private final IAudioService.Stub mInterface = new IAudioService.Stub() {
        ...
        public void load(List<String> mediaPathList, int position, boolean libvlcBacked, boolean noVideo)
            ...
            Log.i("zjj", "AudioService Loading position " + ((Integer)position).toString() + " in " + mediaPathList.toString());
                ...
                for (int i = 0; i < mediaPathList.size(); i++) {
                    ...
                    if(media == null) {
                        if(!validateLocation(location)) {
                        	Log.i("zjj", "AudioService Invalid location " + location);
                            showToast(getResources().getString(R.string.invalid_location, location), Toast.LENGTH_SHORT);
                            continue;
                        }
                        Log.i("zjj", "AudioService Creating on-the-fly Media object for " + location);
        ...
        public void showWithoutParse(String URI) throws RemoteException {
            ...
            mCurrentMedia = new Media(
                    ...
                    null,
                    URI,
                    GlobalVariables.gAppContext.getString(R.string.unknown_artist),
                    GlobalVariables.gAppContext.getString(R.string.unknown_genre),
                    GlobalVariables.gAppContext.getString(R.string.unknown_album),
                    0,
                    0,
                    "",
                    -1,
        ...
        public void append(List<String> mediaLocationList) throws RemoteException {
            if (mMediaList.size() == 0) {
                ...
                Log.i("zjj", "AudioService mMediaList.size()");
                return;
            }

            if(mLibVLCPlaylistActive) {
            	Log.i("zjj", "AudioService mLibVLCPlaylistActive true");
                return;
            }
