...
public class Util {
    ...
    public static String readAsset(String assetName, String defaultS) {
        try {
            InputStream is = GlobalVariables.gAppContext.getResources().getAssets().open(assetName);
    ...
    public static String getValue(String string, int defaultId)
    {
        return (string != null && string.length() > 0) ?
                string : GlobalVariables.gAppContext.getString(defaultId);
    }

    public static int convertPxToDp(int px) {
        WindowManager wm = (WindowManager)GlobalVariables.gAppContext.
    ...
    public static int convertDpToPx(int dp) {
        return Math.round(
                TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dp,
                		GlobalVariables.gAppContext.getResources().getDisplayMetrics())
                         );
    }
    ...
    public static boolean hasCombBar() {
        return (!isPhone()
                && ((android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.HONEYCOMB) &&
                    (android.os.Build.VERSION.SDK_INT <= 16)));//android.os.Build.VERSION_CODES.JELLY_BEAN)));
    }

    public static boolean isPhone(){
        TelephonyManager manager = (TelephonyManager)GlobalVariables.gAppContext.getSystemService(Context.TELEPHONY_SERVICE);
    ...
    public static String[] getCustomDirectories() {
        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(GlobalVariables.gAppContext);
